...
public class SDSFlexDriveConstants implements DriveConstants {
    private static final double ODOMETRY_FREQUENCY = 100.0;

    private static final double TRACK_WIDTH = 0.507;
    private static final double WHEEL_BASE = 0.507;
    ...
    private static final double MAX_ANGULAR_VELOCITY = MAX_VELOCITY / DRIVEBASE_RADIUS * 0.6;
    private static final double MAX_ANGULAR_ACCELERATION = MAX_ACCELERATION / DRIVEBASE_RADIUS * 0.7;
    ...
    // Motor constants
    private static final boolean FRONT_LEFT_INVERTED = SwerveInversions.SDS.MK4n.FRONT_LEFT_INVERTED;
    private static final boolean FRONT_RIGHT_INVERTED = SwerveInversions.SDS.MK4n.FRONT_RIGHT_INVERTED;
    private static final boolean BACK_LEFT_INVERTED = SwerveInversions.SDS.MK4n.BACK_LEFT_INVERTED;
    private static final boolean BACK_RIGHT_INVERTED = SwerveInversions.SDS.MK4n.BACK_RIGHT_INVERTED;
    private static final double WHEEL_DIAMETER = Units.Meter.convertFrom(1.922 * 2, Units.Inch);
    private static final int DRIVE_MOTOR_CURRENT_LIMIT = 100;
    private static final double DRIVE_MOTOR_REDUCTION = GearRatios.SwerveDriveSpecialties.Mk4n.REDUCTION_L1;

    private static final int TURN_MOTOR_CURRENT_LIMIT = 45;
    private static final double TURN_MOTOR_REDUCTION = GearRatios.SwerveDriveSpecialties.Mk4n.STEER_REDUCTION;
    ...
    private static final double TURN_ABS_ENCODER_POSITION_FACTOR = 2 * Math.PI; // Rotations -> Radians
    private static final double TURN_ABS_ENCODER_VELOCITY_FACTOR = (2 * Math.PI) / 60.0; // RPM -> Rad/Sec
    private static final double TURN_REL_ENCODER_POSITION_FACTOR =
            (2 * Math.PI) / TURN_MOTOR_REDUCTION; // Motor rotations -> module radians
    private static final double TURN_REL_ENCODER_VELOCITY_FACTOR =
            (2 * Math.PI) / 60.0 / TURN_MOTOR_REDUCTION; // Motor RPM -> module rad/sec

    // Closed-loop constants
    private static final PIDGains DRIVE_PID = new PIDGains(0.1, 0, 0);
    private static final FeedForwardGains DRIVE_FF = new FeedForwardGains(0.15, 2.4885, 0.5, 0.50);
    private static final PIDGains TURN_PID = new PIDGains(1, 0, 0);
    private static final FeedForwardGains TURN_FF = new FeedForwardGains(0.21058, 0.31057, 0);
    ...
    private final SparkFlexConfig turnConfig = new SparkFlexConfig();

    // Trajectory follower config
    private static final double TRAJECTORY_FOLLOWER_TRANSLATION_KP = 8;
    private static final double TRAJECTORY_FOLLOWER_ROTATION_KP = 6;
    ...
    public SDSFlexDriveConstants() {
        ...
        turnConfig
                .encoder
                .positionConversionFactor(TURN_REL_ENCODER_POSITION_FACTOR)
                .velocityConversionFactor(TURN_REL_ENCODER_VELOCITY_FACTOR)
                .uvwMeasurementPeriod(10)
                .uvwAverageDepth(2);
        turnConfig
                .absoluteEncoder
                .inverted(SwerveInversions.SDS.MK4n.REV_THROUGH_BORE_ENCODER_INVERTED)
                .positionConversionFactor(TURN_ABS_ENCODER_POSITION_FACTOR)
                .velocityConversionFactor(TURN_ABS_ENCODER_VELOCITY_FACTOR)
                .averageDepth(2);
        turnConfig
                .closedLoop
        ...
        turnConfig
                .inverted(SwerveInversions.SDS.MK4n.TURN_MOTOR_INVERTED)
                .idleMode(SparkBaseConfig.IdleMode.kBrake)
